"""Train the triplet network used for timbre-style evaluation."""
...
def load_audio_mfcc(path):
    ...
    -------
    numpy.ndarray
        MFCC feature matrix with the coefficient axis last.
    """
...
def main():
    """Prepare triplet features, train the model, and save weights."""

    model, _ = triplet_network.build_model(num_features=12)

    ta, tb, tc = triplet_network.all_data(
        os.path.join(DATA_DIR, "triplets_train"), load_fn=load_audio_mfcc
    )
    t_size = ta.shape[0]
    ...
    with open(os.path.join(MFCCT_DIR, "train_list"), "w") as f_list:
        for i in tqdm(range(t_size)):
            ...
            for name in ["anchor", "positive", "negative"]:
                ...
                path = f"{str(i).zfill(i_len)}_{name}.npy"
                np.save(os.path.join(MFCCT_DIR, path), example, allow_pickle=False)
                paths.append(path)
            print(*paths, sep="\t", file=f_list)

    va, vb, vc = triplet_network.all_data(
        os.path.join(DATA_DIR, "triplets_val"), load_fn=load_audio_mfcc
    )
    v_size = va.shape[0]
    ...
    train_loader, steps_per_epoch = triplet_network.data_loader(
        os.path.join(MFCCT_DIR, "train_list"),
        load_fn=np.load,
        batch_size=BATCH_SIZE,
        shuffle=True,
        repeat=True,
    )
    val_loader, _ = triplet_network.data_loader(
        os.path.join(MFCCV_DIR, "val_list"), load_fn=np.load, batch_size=1
    )
